...
static std::vector<char> readFile(const std::string& filename) {
	...
	// Record filesize from buffer position
...
private:
	...
	void InitVulkan() {
		...
		std::cout << "\nRequired Extensions(" << glfwExtensionCount << "):\n";
	...
	void PickPhysicalDevice() {
		...
		// Logging Devices
		std::cout << "\nDevices w/Vulkan Support (" << deviceCount << "):\n";
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		// Checking if required queue families are available
		QueueFamilyIndices indices = findQueueFamilies(device);
		
		// Checking if required device extensions are available
		bool extensionsSupported = checkDeviceExtensionSupport(device);

		// Checking if the swap chain is available and support is adequate
	...
	void CreateLogicalDevice() {
		...
		// Add validation layers to logical device for older Vulkan versions
		//   modern versions of vulkan ignore this
		if (enableValidationLayers) {
			createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
			createInfo.ppEnabledLayerNames = validationLayers.data();
		} else {
	...
	void Cleanup() {
		...
		ConsoleLog("Vulkan and GLFW cleaned up");
	}
